...
def train_deep_sdf(
    ...
    with get_profiler(config) as profiler:
        ...
        for epoch in range(config['resume_epoch'] + 1, config['n_epochs'] + 1):
            ...
            log_dict = train_epoch(model, data_loader, latent_vecs, optimizer=optimizer, config=config, epoch=epoch, return_loss=True, n_surfaces=config["objects_per_decoder"])
